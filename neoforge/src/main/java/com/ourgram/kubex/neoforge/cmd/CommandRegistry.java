...
import com.ourgram.kubex.command.CommandStatus;
...
public final class CommandRegistry {
    ...
    public CommandRegistry(CommandService commandService) {
    ...
    private void register(CommandDispatcher<CommandSourceStack> dispatcher, Predicate<CommandSourceStack> requirement) {
        dispatcher.register(
            ...
            .then(
                ...
                .then(Commands.literal("js").executes(context -> init(context.getSource(), InitMode.JS)))
                .then(Commands.literal("ts").executes(context -> init(context.getSource(), InitMode.TS)))
            ).then(
    ...
    private int init(CommandSourceStack source, InitMode mode) {
    ...
    private int export(CommandSourceStack source) {
        Path gameRoot = resolveGameRoot(source);
